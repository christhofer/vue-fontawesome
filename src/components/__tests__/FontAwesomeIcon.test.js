...
import { faCoffee, faCircle, farGem, fasGem } from '../__fixtures__/icons'
...
describe('set prefix', () => {
  library.add(farGem, fasGem)
    
  test('default prefix is fas', () => {
    const vm = mountFromProps({ icon: 'gem' })

    expect(vm.$el.tagName).toBe('svg')
    expect(vm.$el.classList.contains('fa-gem')).toBeTruthy()
